...
export function FaqAccordion() {
  ...
  return (
    <div className="grid gap-4">
      {faqs.map((faq, index) => (
        <div key={faq.question} className="rounded-2xl border border-white/10 bg-white/[0.035] shadow-[0_0_45px_rgba(37,99,235,0.08)]">
          <button
          ...
          <AnimatePresence initial={false}>
            {active === index ? (
              ...
              >
                <p className="px-6 pb-6 leading-8 text-white/60">{faq.answer}</p>
              </motion.div>
            ) : null}
          </AnimatePresence>
        </div>
