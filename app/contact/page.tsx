...
export async function generateMetadata(): Promise<Metadata> {
  const { t } = getT();
  return {
    title: t("page.contact.metaTitle"),
    description: t("page.contact.metaDescription"),
  };
}
...
export default function ContactPage() {
  const { t } = getT();
  const contactItems = [
    { icon: Phone, label: t("page.contact.phone"), value: siteConfig.phone, href: `tel:${siteConfig.phone}` },
    { icon: Mail, label: t("page.contact.email"), value: siteConfig.email, href: `mailto:${siteConfig.email}` },
    { icon: MapPin, label: t("page.contact.address"), value: t("site.address"), href: null },
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-black text-white">
      <PageHero
        title={t("page.contact.heroTitle")}
        description={t("page.contact.heroDescription")}
      />
        ...
        <div className="container-osb grid gap-8 lg:grid-cols-2">
          <div className="grid gap-5">
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-6 shadow-[0_0_45px_rgba(37,99,235,0.08)]">
              <p className="font-bold text-white">{t("page.contact.workingHours")}</p>
              <p className="mt-2 text-white/55">{t("site.workingHours")}</p>
            </div>
          </div>
