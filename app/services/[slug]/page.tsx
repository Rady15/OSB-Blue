...
export function generateMetadata({ params }: ServicePageProps): Metadata {
  ...
  const title = t(`service.${service.slug}.title`);
  const description = t(`service.${service.slug}.problemParagraphs`).split("|")[0];
...
export default function ServiceDetailPage({ params }: ServicePageProps) {
  ...
  const title = t(`service.${service.slug}.title`);
  const heroQuestion = t(`service.${service.slug}.heroQuestion`);
  const shortDescription = t(`service.${service.slug}.shortDescription`);
  const problemParagraphs = t(`service.${service.slug}.problemParagraphs`).split("|");
  const solutionParagraph = t(`service.${service.slug}.solutionParagraph`);
  const suitableIf = t(`service.${service.slug}.suitableIf`).split("|");

  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "Service",
    name: title,
    description: problemParagraphs[0],
    ...
    <div className="min-h-screen overflow-hidden bg-black text-white">
      ...
      <section className="relative overflow-hidden bg-black pb-20 pt-36 text-white">
        ...
        <div className="container-osb relative z-10">
          <p className="mb-5 text-sm font-bold text-white/55">{title}</p>
          <ScrollRevealText
            as="h1"
            text={heroQuestion}
            wordsPerLine={4}
          ...
          <div className="mt-10 rounded-[1.5rem] border border-white/10 bg-black p-5">
            <div className="grid gap-6 lg:grid-cols-[1fr_1fr] lg:items-center lg:gap-10">
              <div className="grid gap-4">
                ...
                <p className="leading-8 text-white/55">{shortDescription || heroQuestion}</p>
              </div>
          ...
          <div className="grid gap-6">
            ...
            <div className="glow-panel rounded-[2rem] border border-white/10 bg-white/[0.04] p-4">
              <div className="rounded-[1.5rem] border border-white/10 bg-black p-7">
                <ScrollRevealText as="h2" text={t("service.howWeHelp")} className="text-2xl font-extrabold text-white" />
                <p className="mt-4 text-lg leading-9 text-white/60">{solutionParagraph}</p>
              </div>
            </div>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-7 shadow-[0_0_45px_rgba(37,99,235,0.08)]">
            <ScrollRevealText as="h2" text={t("service.suitableIfTitle")} className="text-2xl font-extrabold text-white" />
      ...
      <section className="border-y border-white/5 bg-[#111] py-16">
        <div className="container-osb">
          ...
          <ScrollRevealText as="h2" text={t("service.cta.title")} wordsPerLine={6} className="text-3xl font-extrabold" />
          <p className="mt-4 max-w-3xl text-lg leading-9 text-white/60">{t("service.cta.description")}</p>
          <div className="mt-8">
            <LinkButton href="/free-consultation">{t("service.cta.button")}</LinkButton>
          </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-black py-16">
        <div className="container-osb">
          <ScrollRevealText as="h2" text={t("service.otherServices")} className="mb-6 text-2xl font-extrabold text-white" />
